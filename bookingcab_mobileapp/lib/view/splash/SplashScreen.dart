...
class SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> isLanguageSelctedOrNot() async {
    ...
    if (!isLoginOrNot) {
      ...
      USER_ID = await SharedPreferencesUtil.getDataBykey(SharedPreferencesUtil.KEY_USER_ID);
        ...
        if (response.statusCode == 200) {
         ...
         // hideCustomeLoader(context);
          if (responseData.status == SUCCESS_STATUS) {
              ...
              USER_SIGNUP_STATUS = await SharedPreferencesUtil.getDataBykey(SharedPreferencesUtil.KEY_USER_SIGNUP_STATUS);
              COUNTRY_ID = userProfileInfoData!.companyId.toString();
              USER_TOCKEN = await SharedPreferencesUtil.getDataBykey(SharedPreferencesUtil.KEY_USER_TOCKEN);
